...
//  OPTIMIZED: Works directly with buffer data to avoid copying 100MB payloads.
//
//  CGI format expected:
//    Status: 200 OK\r\n        (optional)
//    Content-Type: text/html\r\n
    ...
    Buffer&             wb)
{
    ...
    // Find header/body separator — scan directly, no copy
    ...
    // Parse CGI headers from the buffer directly (no copy)
    int         status_code   = 200;
    ...
    // Body is everything after the separator - append directly from buffer
    size_t body_size = size - (sep + body_skip);
    
    // Write headers first
    _writeHeaders(status_code, content_type, body_size, extra_headers, req, wb);
    
    // Append body directly from the original buffer (no copy!)
